...
import com.cincc.o2o.dao.ShopDao;
...
import com.cincc.o2o.util.PageCalculator;
...
public class ShopServiceImpl implements ShopService {
    ...
    public ShopExecution getShopList(Shop shopCondition, int pageIndex, int pageSize) {
        ...
        if (shopList!=null){
            ...
            se.setState(ShopStateEnum.INNER_ERRO.getState());
        }
        return se;
    }
    ...
    public ShopExecution addshop(Shop shop, ImageHolder thumbnail){
        ...
        try {
            ...
            if (effectedNum<=0){
                //使用RunTimeException抛出异常是因为只有抛出RunTimeException
                //或继承了RunTimeException的类时事务才能截止并回滚
                throw new ShopOperationException("店铺创建失败");
            }else {
                if (thumbnail.getImage() != null) {
                    //存储图片
                    //ctrl+alt+t 选择surround with方法
                    try {
    ...
    public ShopExecution modifyShop(Shop shop, ImageHolder thumbnail) throws ShopOperationException {
        if (shop==null || shop.getShopId()==null ){
            ...
            try {
                ...
                if (effectedNum<=0){
                    return new ShopExecution(ShopStateEnum.INNER_ERRO);
                }else {
